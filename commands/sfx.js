...
async function getFormattedSoundsList() {
  return (await soundsUtils.getAvailableSounds())
    .sort((a, b) => a.localeCompare(b))
    .reduce(soundsUtils.formatFileNames, [])
}

async function getCommandDescription() {
...
Sounds:
${await getFormattedSoundsList()}\n\n`

  return commandDescription;
}

async function updateSoundsListInGlobalCommands() {
  const index = global.commandsList.findIndex(c => c.id === 'soundsList');
  const commandDescription = await getCommandDescription();
  global.commandsList[index] = { id: 'soundsList', commandDescription };
}

addCommandsToList = async () => {
  const commandDescription = await getCommandDescription();
  global.commandsList.push({ id: 'soundsList', commandDescription });
}


addCommandsToList();

module.exports = { playSound, updateSoundsListInGlobalCommands };